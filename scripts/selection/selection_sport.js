/**
 * Script SPORT Cop1 Ã¢â‚¬â€œ SÃƒÂ©lection simplifiÃƒÂ©e
 *
...
function exclureBlacklistEtSelectionAleatoire() {
  ...
  var historiqueSS     = SpreadsheetApp.openById(idFichierHistorique);
  var historiqueSheet  = historiqueSS.getSheetByName(nomFeuilleHistorique);
  if (!historiqueSheet) throw new Error(`Ã¢ÂÅ’ "${nomFeuilleHistorique}" introuvable`);

  /************************************************************
   * (3) INDEX DES COLONNES Ã¢â‚¬â€œ SPORT
   ************************************************************/
  var idxDateR    = 0; // A : Horodateur
  var idxPrenomR  = 4; // E : PrÃƒÂ©nom
  var idxNomR     = 5; // F : Nom
  var idxTelR     = 6; // G : TÃƒÂ©lÃƒÂ©phone

  var idxTelBL    = 2; // C
  var idxAnnulBL  = 7; // H : Nombre Croix

  /************************************************************
   * (4) LECTURE DES DONNÃƒâ€°ES
   ************************************************************/
  // 4.1 RÃƒÂ©ponses
  var lastRowR = reponsesSheet.getLastRow();
  var lastColR = reponsesSheet.getLastColumn();
  if (lastRowR < 2){ Logger.log("Ã¢Å¡Â Ã¯Â¸Â Pas de rÃƒÂ©ponses"); return; }
  ...
  /************************************************************
  * (5.3) DÃƒÂ©jÃƒÂ  sÃƒÂ©lectionnÃƒÂ©s <14 jours
  ************************************************************/
  var recentSet = new Set();
  var limite = new Date(); limite.setDate(limite.getDate() - 14);
  historiqueData.forEach(function(r){
    var dat = new Date(r[0]), tel = normalizeTel(r[3]);
    if (tel && dat >= limite && !blacklistSet.has(tel)) {
      recentSet.add(tel);
    }
  });
  ...
  /************************************************************
   * (6) DÃƒâ€°DUPLICATION Ã¢â‚¬â€œ garder la plus rÃƒÂ©cente
   ************************************************************/
  var latestMap = {};
  reponsesData.forEach(function(r){
    var tel = normalizeTel(r[idxTelR]), dt = new Date(r[idxDateR]);
    if (!tel) return;
    if (!latestMap[tel] || dt > latestMap[tel].date){
      latestMap[tel] = { row: r, date: dt };
    }
  });

  /************************************************************
   * (7) FILTRAGE & LOG DÃƒâ€°TAILLÃƒâ€°
   ************************************************************/
  var totalCandidats = Object.keys(latestMap).length;
  var ok = [], recents = [], blacklistÃƒÂ©s = 0;

  Object.values(latestMap).forEach(function(o){
  ...
  ************************************************************/
  selectionSheet.clearContents();
    selectionSheet.getRange(1,1,1,4).setValues([["Horodateur", "Nom", "PrÃƒÂ©nom", "TÃƒÂ©lÃƒÂ©phone"]]);
...
if (finalList.length){
  ...
  finalList.forEach(function(r){
    ...
    historiqueSheet.appendRow([today, nom, pren, tel, ss.getName()]);
  });
}
...
function normalizeTel(raw){
  ...
  return ''; // Invalide ou mal formÃƒÂ©
}
